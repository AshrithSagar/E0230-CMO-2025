...
Since \( \mathbf{x}^* \) is in the interior of \( S \), there exists a radius \( r > 0 \) such that the open ball \( B_r(\mathbf{x}^*) = \{ \mathbf{z} \in S \;\big|\; \|\mathbf{z} - \mathbf{x}^*\| < r \} \ \subseteq S \), by the definition of an interior point.
