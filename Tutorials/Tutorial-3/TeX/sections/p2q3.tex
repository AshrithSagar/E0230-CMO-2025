...
For Newton's method, we need the function to once differentiable, and twice differentiable at the point of interest.

Alternative methods available in these cases: Sub-gradient methods, Proximal gradient methods.
...
As can be seen from the iterates, starting with \( x_0 = 1 (> 0) \) leads to \( x_1 = -\half (< 0) \), and starting with \( x_0 = -1 (< 0) \) leads to \( x_1 = \half (> 0) \).
