...
An \textbf{unconstrained optimisation problem} is the problem of finding a point \( \mathbf{x}^* \in \R^d \) that minimises a function \( f: \R^d \to \R \), i.e.,
\begin{equation*}
    f(\mathbf{x}^*) = \min_{\mathbf{x} \in \R^d} f(\mathbf{x})
    \quad \iff \quad
    \mathbf{x}^* = \argmin_{\mathbf{x} \in \R^d} f(\mathbf{x})
    \quad \iff \quad
    f(\mathbf{x}^*) \leq f(\mathbf{x}), \quad \forall \mathbf{x} \in \R^d
\end{equation*}
...
Note that \( \because f(t) \geq 0, \; \forall t \in \R \implies f(t^*) \geq 0 \), which gives the Cauchy-Schwarz inequality~\psecref{sec:cauchy-schwarz-inequality}.
