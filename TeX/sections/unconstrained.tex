...
\begin{definition}{\( \Q \)--conjugates}{}
    Given some real symmetric positive definite matrix \( \Q \in \PD \), a set of vectors \( \set{ \u_i }_{i = 1}^n, \; \u_i \in \R^d \) are said to be \textbf{conjugates with respect to \( \Q \)} \text{ OR } \textbf{\( \Q \)--conjugates} if
    \vspace{-0.5em}
    \begin{equation*}
        \qf{\u_i}{\Q}[\u_j] = 0,
...
\begin{proof}
    Let \( \set{ \u_i }_{i = 1}^n, \; \u_i \in \R^d \) be a set of \( \Q \)--conjugate vectors.
    ...
    Since \( \qf{\u_l}{\Q} > 0, \; \because \Q \succ \zero \), we have that \( \beta_i = 0, \; \forall i \in \set{1, 2, \ldots, n} \).
