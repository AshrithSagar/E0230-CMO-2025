...
One such choice for \( \mathbf{p}^{(k)} \in \mathcal{DS} \left( \mathbf{x}^{(k)} \right) \) is the negative gradient direction,
\begin{equation*}
    \mathbf{p}^{(k)} = -\nabla f(\mathbf{x}^{(k)})
\end{equation*}
...
\begin{algorithm}[H]
    ...
    \While{\( \mathbf{x}^{(k)} \) is not optimal}{
        Choose a descent direction \( \mathbf{p}^{(k)} \in \mathcal{DS} \left( \mathbf{x}^{(k)} \right) \)\;
...
\begin{equation*}
    ...
    \rho^k E_0
\end{equation*}
where \( \rho \in [0, 1) \) since \( \mathbf{Q} \succ \mathbf{0} \implies \lambda_{\max}(\mathbf{Q}) \geq \lambda_{\min}(\mathbf{Q}) > 0 \), and \( 4ab \leq (a+b)^2, \; \forall a, b \).
