...
Assume \( f \in \C^{1} \), and use \( f(\x + \p) = f(\x) + {\nabla f(\x)}^\top \p + o(\normbf{p}) \).

\subsection{Second-order methods}

Assume \( f \in \C^{2} \), and use \( f(\x + \p) = f(\x) + {\nabla f(\x)}^\top \p + \half \p^\top \nabla^{2} f(\x) \p + o(\normbf{p}^2) \).
...
\begin{definition}{Global minimum}{global-minimum}
    ...
    \begin{equation*}
        \func{f}{\xstar} \leq f(\x), \quad \forall \x \in D
    \end{equation*}
\end{definition}
...
\begin{definition}{Local minimum}{}
    The point \( \xstar \in \R^d \) is a \textbf{local minimum} of the function \( f: \R^d \to \R \) if there exists a \( \delta > 0 \) such that for all \( \x \) in the \( \delta \)-neighborhood of \( \xstar \), we have \( \func{f}{\xstar} \leq f(\x) \), i.e.,
    \begin{equation*}
        \func{f}{\xstar} \leq f(\x), \quad \forall \x \in B_{\delta}(\xstar)
    \end{equation*}
\end{definition}
...
\begin{definition}{Strict local minimum}{}
    The point \( \xstar \in \R^d \) is a \textbf{strict local minimum} of the function \( f: \R^d \to \R \) if there exists a \( \delta > 0 \) such that for all \( \x \) in the \( \delta \)-neighborhood of \( \xstar \) except \( \xstar \) itself, we have \( \func{f}{\xstar} < f(\x) \), i.e.,
    \begin{equation*}
        \func{f}{\xstar} < f(\x), \quad \forall \x \in B_{\delta}(\xstar) \setminus \{ \xstar \}
    \end{equation*}
\end{definition}
...
\begin{theorem}{Second-order necessary condition for a local minimum}{}
    For a function \( f \in \C^{2} \), if \( \xstar \) is a local minimum of \( f \), then \( \grad{f}{\xstar} = \zero, \; \nabla^2 \func{f}{\xstar} \succeq 0 \).
\end{theorem}
...
\begin{theorem}{Second-order sufficient condition for a strict local minimum}{}
    For a function \( f \in \C^{2} \) such that \( \grad{f}{\xstar} = \zero, \; \nabla^2 \func{f}{\xstar} \succ 0 \), then \( \xstar \) is a strict local minimum of \( f \).
\end{theorem}
...
\begin{proof}
    ...
    \begin{equation*}
        \frac{f(\lambda \x + (1 - \lambda) \y) - f(\x)}{\lambda}
        \leq
        f(\y) - f(\x),
        \quad \forall \x, \y \in D,
    ...
    \begin{equation*}
        {\nabla f(\x)}^\top (\y - \x)
        =
        \lim_{\lambda \to 0^+} \frac{f(\lambda \x + (1 - \lambda) \y) - f(\x)}{\lambda}
        \leq
        f(\y) - f(\x),
        \quad \forall \x, \y \in D
    \end{equation*}
...
\begin{corollary}{Monotonicity of gradient \( \iff \) Convexity}{}
    For a function \( f \in \C^1 \) with a convex domain \( D \), \( f \) is convex on \( D \) iff
    \begin{equation*}
        {\big( \nabla f(\x) - \nabla f(\y) \big)}^\top \big( \x - \y \big) \geq 0
        , \quad
        \forall \x, \y \in D
    \end{equation*}
\end{corollary}
...
\begin{proof}
    ...
    \( (\Leftarrow) \)
    Define \( g(t) = f(\x + t (\y - \x)), \;\; t \in [0, 1], \;\; \x, \y \in D \).

    Then, \( g(0) = f(\x), \quad g(1) = f(\y), \quad g'(t) = {(\nabla f(\x + t (\y - \x)))}^\top (\y - \x) \).
...
\begin{proof}
    ...
    Therefore, we have
    \begin{equation*}
        {(\y - \x)}^\top \nabla^2 f(\boldsymbol{\xi}) (\y - \x) \geq 0
        , \quad
        \forall \x, \y \in D
    \end{equation*}
    ...
    \end{equation*}
    for some \( t \in (0, 1) \).
    Since \( \nabla^2 f(\x) \succeq 0, \; \forall \x \in D \), we have
    \begin{equation*}
        f(\y) - f(\x) - {\nabla f(\x)}^\top (\y - \x) \geq 0
        , \quad
        \forall \x, \y \in D
...
\begin{corollary}{}{}
    A function \( f: D \subseteq \R^n \to \R \) on a convex domain \( D \) is strongly convex with parameter \( \mu > 0 \) iff the function \( g(\x) := \left( f(\x) - \frac{\mu}{2} \norm{\x}^2 \right) \) is convex on \( D \).
\end{corollary}
...
\begin{corollary}{Second-order condition for strong convexity}{}
    ...
    \begin{equation*}
        ...
        \forall \x \in D
    \end{equation*}
    i.e., the Hessian \( \nabla^2 f(\x) \) is positive definite with all eigenvalues at least \( \mu \), for all \( \x \in D \).
\end{corollary}

\begin{proof}
    Define \( g(\x) = f(\x) - \frac{\mu}{2} \norm{\x}^2 \).
    Then, \( \nabla^2 g(\x) = \nabla^2 f(\x) - \mu \mathbf{I} \).
    From the second-order condition for convexity~\pthmref{thm:second-order-condition-for-convexity}, we have that \( g \) is convex iff \( \nabla^2 g(\x) \succeq 0, \; \forall \x \in D \), which is equivalent to the desired result.
\end{proof}
...
\begin{itemize}
    \item \textbf{Zero-order oracle:} Given \( \x \in \R^d \), returns the function value \( f(\x) \in \R \).

    \item \textbf{First-order oracle:} Given \( \x \in \R^d \), returns a tuple \( \big( f(\x), \nabla f(\x) \big) \in (\R, \R^d) \).

    \item \textbf{Second-order oracle:} Given \( \x \in \R^d \), returns a tuple \( \big( f(\x), \nabla f(\x), \nabla^2 f(\x) \big) \in (\R, \R^d, \R^{d \times d}) \).
\end{itemize}
...
\begin{algorithm}[H]
    ...
    \SetAlgoLined{}
    \KwIn{
        First-order oracle for the objective function \( f(\x) \);
        Initial point \( \x^{(0)} \in \R^d \);
    }
    \KwOut{
        Approximate solution to the unconstrained minimisation problem \( \displaystyle \argmin_{\x \in \R^d} f(\x) \)\;
    }
...
\begin{align*}
    \min_{\x \in \R^d} f(\x),
    \quad \text{where }
    f(\x) = \left( \half \qfbf{x}{Q} + \dotp{\h}{\x} + c \right),
...
\paragraph{Solution:}
The gradient of \( f \) can be computed as \( \nabla f(\x) = \Q \x + \mathbf{h} \), and thereby from the first-order necessary condition for local minimum, the local minimum \( \xstar \) must necessarily satisfy
\begin{equation*}
