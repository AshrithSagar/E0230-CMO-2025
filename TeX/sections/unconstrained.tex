...
We have \( \A_{k+1} \succ \zero \) if \( \A_k \succ \zero \) and \( \beta > 0 \).
...
\begin{equation*}
    ...
    \quad \beta_1, \beta_2, \beta_3 \in \R_{>0}
\end{equation*}
...
Observe that for \( \phi = 1 \), we get the DFP update~\psecref{sec:dfp}, and for \( \phi = 0 \), we get the rank-1 update~\psecref{sec:rank-1-update}.
