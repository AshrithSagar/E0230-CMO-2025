...
\begin{equation*}
    \boxed{
        ...
        \beta
        =
        \frac{1}{\dotp{\v}{\boldsymbol{\gamma}_k}}
    }
\end{equation*}

Hence, we have the rank-1 update as
\begin{equation*}
    \boxed{
        \A_{k+1}
        =
...
\section{Rank-2 update (DFP)}\label{sec:dfp}
...
\begin{theorem}{DFP positive definiteness}{}
    If \( \A_0 \succ \zero \) and \( \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k} > 0, \; \forall k \), then the DFP update preserves positive definiteness, i.e.,\\
    If \( \A_k \succ \zero \) and \( \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k} > 0 \), then \( \A_{k+1} \succ \zero \).
\end{theorem}
