...
\begin{align*}
    ...
    \quad \Q \succ 0, \text{i.e.}, \Q \in \mathbf{S}_d^{++},
    \quad \mathbf{h} \in \R^d,
    \quad c \in \R
\end{align*}

\paragraph{Solution:}
The gradient of \( f \) can be computed as \( \grad{f}{\x} = \Q \x + \mathbf{h} \), and thereby from the first-order necessary condition for local minimum, the local minimum \( \xstar \) must necessarily satisfy
\begin{equation*}
...
\begin{align*}
    ...
    \func{f}{\xk + \alpha \pk}
    \\ & =
    \half {\left( \xk + \alpha \pk \right)}^\top \Q \left( \xk + \alpha \pk \right)
    + \mathbf{h}^\top \left( \xk + \alpha \pk \right)
    + c
    \\ & =
    \half {\xk}^\top \Q \xk
    + \alpha {\xk}^\top \Q \pk
    + \frac{\alpha^2}{2} {\pk}^\top \Q \pk
    + \mathbf{h}^\top \xk
    + \alpha \mathbf{h}^\top \pk
    + c
    \\ & =
    \alpha^2 \left( \half {\pk}^\top \Q \pk \right)
    + \alpha \left( {\xk}^\top \Q \pk + \mathbf{h}^\top \pk \right)
    + \left( \half {\xk}^\top \Q \xk + \mathbf{h}^\top \xk + c \right)
    \\ & =
    \alpha^2 \underbrace{ \left( \half {\pk}^\top \Q \pk \right) }_{p_k}
    + \alpha \underbrace{ \left( {\grad{f}{\xk}}^\top \pk \right) }_{q_k}
    + \underbrace{ \left( \func{f}{\xk} \right) }_{r_k}
\end{align*}
which is a quadratic function in \( \alpha \) with \( p_k > 0 \; \left( \because \Q \succ 0 \right) \) and \( q_k < 0 \; \left( \because \pk \in \func{\mathcal{DS}}{\xk} \right) \).
...
\begin{equation*}
    ...
    \boxed{
        \alpha_k
        = -\frac{{\grad{f}{\xk}}^\top \pk}{{\pk}^\top \Q \pk}
    }
    > 0
\end{equation*}
\begin{equation*}
    ...
    \boxed{
        \func{f}{\xkp}
        =
        \func{f}{\xk} - \frac{{\left( {\grad{f}{\xk}}^\top \pk \right)}^2}{2 {\pk}^\top \Q \pk}
    }
\end{equation*}
...
\begin{align*}
    ...
    =
    \frac{{\left( {\grad{f}{\xk}}^\top \pk \right)}^2}{2 {\pk}^\top \Q \pk}
    \\
    \implies
    \frac{E_{k} - E_{k+1}}{E_k}
    & =
    \frac{{\left( {\grad{f}{\xk}}^\top \pk \right)}^2}{ {\left( \grad{f}{\xk} \right)}^\top \Qinv \, \grad{f}{\xk} \; {\pk}^\top \Q \pk}
\end{align*}
...
\begin{equation*}
    ...
    \rho^k E_0
\end{equation*}
where \( \rho \in \rinterval{0}{1} \) since \( \Q \succ 0 \implies \func{\lambda_{\max}}{\Q} \geq \func{\lambda_{\min}}{\Q} > 0 \), and \( 4ab \leq {(a+b)}^2, \; \forall a, b \).
