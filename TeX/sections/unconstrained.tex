...
\begin{definition}{Strong convexity}{}
    A function \( f: D \subseteq \mathbb{R}^n \to \mathbb{R} \) on a convex domain \( D \) is \textbf{strongly convex} with parameter \( \mu > 0 \) if
    \begin{equation*}
        f(\lambda \mathbf{x} + (1 - \lambda) \mathbf{y})
        \leq
        \lambda f(\mathbf{x}) + (1 - \lambda) f(\mathbf{y})
        - \frac{\mu}{2} \lambda (1 - \lambda) \Vert \mathbf{x} - \mathbf{y} \Vert^2,
        \quad \forall \mathbf{x}, \mathbf{y} \in \mathbb{R}^n, \; \lambda \in [0, 1]
    \end{equation*}
\end{definition}

\begin{corollary}{}{}
    A function \( f: D \subseteq \mathbb{R}^n \to \mathbb{R} \) on a convex domain \( D \) is strongly convex with parameter \( \mu > 0 \) iff the function \( g(\mathbf{x}) := \left( f(\mathbf{x}) - \frac{\mu}{2} {\Vert \mathbf{x} \Vert}^2 \right) \) is convex on \( D \).
\end{corollary}

\begin{proof}
    Follows from the identity:
    \begin{align*}
        \lambda (1 - \lambda) \Vert \mathbf{x} - \mathbf{y} \Vert^2
        & =
        \lambda (1 - \lambda) \Big( {\Vert \mathbf{y} \Vert}^2 + {\Vert \mathbf{x} \Vert}^2 - 2 \mathbf{x}^{\top} \mathbf{y} \Big)
        \\ & =
        \lambda {\Vert \mathbf{y} \Vert}^2 + \lambda {\Vert \mathbf{x} \Vert}^2 - \lambda^2 {\Vert \mathbf{y} \Vert}^2 - {\Vert \lambda \mathbf{x} \Vert}^2 - 2 {(\lambda \mathbf{x})}^{\top} \big( (1 - \lambda) \mathbf{y} \big)
        \\ & =
        \lambda {\Vert \mathbf{y} \Vert}^2 + \lambda {\Vert \mathbf{x} \Vert}^2 - \lambda^2 {\Vert \mathbf{y} \Vert}^2 - {\Vert \lambda \mathbf{x} + (1 - \lambda) \mathbf{y} \Vert}^2 + {(1 - \lambda)}^2 {\Vert \mathbf{y} \Vert}^2
        \\ & =
        \cancel{ \lambda {\Vert \mathbf{y} \Vert}^2 } + \lambda {\Vert \mathbf{x} \Vert}^2 - \cancel{ \lambda^2 {\Vert \mathbf{y} \Vert}^2 } - {\Vert \lambda \mathbf{x} + (1 - \lambda) \mathbf{y} \Vert}^2 + {\Vert \mathbf{y} \Vert}^2 - \cancel{ 2 } \lambda {\Vert \mathbf{y} \Vert}^2 + \cancel{ \lambda^2 {\Vert \mathbf{y} \Vert}^2 }
        \\ & =
        \lambda {\Vert \mathbf{x} \Vert}^2 + (1 - \lambda) {\Vert \mathbf{y} \Vert}^2 - {\Vert \lambda \mathbf{x} + (1 - \lambda) \mathbf{y} \Vert}^2
    \end{align*}
\end{proof}
...
\begin{corollary}{First-order condition for strong convexity}{}
    For a function \( f \in \mathcal{C}^1 \) with a convex domain \( D \), \( f \) is strongly convex on \( D \) with parameter \( \mu > 0 \) iff
    \begin{equation*}
        f(\mathbf{y})
        \geq
        f(\mathbf{x}) + {\nabla f(\mathbf{x})}^\top (\mathbf{y} - \mathbf{x}) + \frac{\mu}{2} \Vert \mathbf{y} - \mathbf{x} \Vert^2,
        \quad \forall \mathbf{x}, \mathbf{y} \in D
    \end{equation*}
\end{corollary}

\begin{proof}
    Define \( g(\mathbf{x}) = f(\mathbf{x}) - \frac{\mu}{2} {\Vert \mathbf{x} \Vert}^2 \).
    ...
    \begin{align*}
        ...
        \\
        \implies
        f(\mathbf{y}) - \frac{\mu}{2} {\Vert \mathbf{y} \Vert}^2
        & \geq
        f(\mathbf{x}) - \frac{\mu}{2} {\Vert \mathbf{x} \Vert}^2 + {\big( \nabla f(\mathbf{x}) - \mu \mathbf{x} \big)}^\top (\mathbf{y} - \mathbf{x})
        \\
        \implies
        f(\mathbf{y})
        & \geq
        f(\mathbf{x}) + {\nabla f(\mathbf{x})}^\top (\mathbf{y} - \mathbf{x}) + \frac{\mu}{2} \Big( {\Vert \mathbf{y} \Vert}^2 - {\Vert \mathbf{x} \Vert}^2 - 2 \mathbf{x}^\top (\mathbf{y} - \mathbf{x}) \Big)
        \\ & =
        f(\mathbf{x}) + {\nabla f(\mathbf{x})}^\top (\mathbf{y} - \mathbf{x}) + \frac{\mu}{2} \Big( {\Vert \mathbf{y} \Vert}^2 - \cancel{ {\Vert \mathbf{x} \Vert}^2 } - 2 \mathbf{x}^\top \mathbf{y} + \cancel{2} {\Vert \mathbf{x} \Vert}^2 \Big)
        \\ & =
        f(\mathbf{x}) + {\nabla f(\mathbf{x})}^\top (\mathbf{y} - \mathbf{x}) + \frac{\mu}{2} {\Vert \mathbf{y} - \mathbf{x} \Vert}^2
        , \quad
        \forall \mathbf{x}, \mathbf{y} \in D
...
\begin{corollary}{Second-order condition for strong convexity}{}
    For a function \( f \in \mathcal{C}^2 \) with a convex domain \( D \), \( f \) is strongly convex on \( D \) with parameter \( \mu > 0 \) iff
    \begin{equation*}
        \nabla^2 f(\mathbf{x})
        \succeq
        \mu \mathbf{I}
        ...
        \forall \mathbf{x} \in D
    \end{equation*}
    i.e., the Hessian \( \nabla^2 f(\mathbf{x}) \) is positive definite with all eigenvalues at least \( \mu \), for all \( \mathbf{x} \in D \).
\end{corollary}

\begin{proof}
    Define \( g(\mathbf{x}) = f(\mathbf{x}) - \frac{\mu}{2} {\Vert \mathbf{x} \Vert}^2 \).
    Then, \( \nabla^2 g(\mathbf{x}) = \nabla^2 f(\mathbf{x}) - \mu \mathbf{I} \).
    From the second-order condition for convexity~\pthmref{thm:second-order-condition-for-convexity}, we have that \( g \) is convex iff \( \nabla^2 g(\mathbf{x}) \succeq 0, \; \forall \mathbf{x} \in D \), which is equivalent to the desired result.
\end{proof}
...
An \textbf{oracle} is a procedure that provides information about the objective function \( f: \mathbb{R}^d \to \mathbb{R} \) at a given point \( \mathbf{x} \in \mathbb{R}^d \).
An \( n \)-th order oracle provides information up to the \( n \)-th derivative of \( f \) at \( \mathbf{x} \).
\begin{itemize}
    \item \textbf{Zero-order oracle:} Given \( \mathbf{x} \in \mathbb{R}^d \), returns the function value \( f(\mathbf{x}) \in \mathbb{R} \).

    \item \textbf{First-order oracle:} Given \( \mathbf{x} \in \mathbb{R}^d \), returns a tuple \( \big( f(\mathbf{x}), \nabla f(\mathbf{x}) \big) \in (\mathbb{R}, \mathbb{R}^d) \).

    \item \textbf{Second-order oracle:} Given \( \mathbf{x} \in \mathbb{R}^d \), returns a tuple \( \big( f(\mathbf{x}), \nabla f(\mathbf{x}), \nabla^2 f(\mathbf{x}) \big) \in (\mathbb{R}, \mathbb{R}^d, \mathbb{R}^{d \times d}) \).
\end{itemize}
...
\paragraph{Solution:}
The gradient of \( f \) can be computed as \( \nabla f(\mathbf{x}) = \mathbf{Q} \mathbf{x} + \mathbf{h} \), and thereby from the first-order necessary condition for local minimum, the local minimum \( \mathbf{x}^* \) must necessarily satisfy
\begin{equation*}
...
\begin{equation*}
    \frac{f(\mathbf{x}^{(k)}) - f(\mathbf{x}^{(k+1)})}{{\Vert \nabla f(\mathbf{x}^{(k)}) \Vert}^2}
    \geq R_k \geq R > 0
\end{equation*}

\section{Global convergence criterion}

If a scheme generates a sequence \( {\{ \mathbf{x}^{(k)} \}}_{k=0}^{\infty} \) such that
\begin{equation*}
    \lim_{k \to \infty} {\Vert \nabla f(\mathbf{x}^{(k)}) \Vert} = 0
\end{equation*}
