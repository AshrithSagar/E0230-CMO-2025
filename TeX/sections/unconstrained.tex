...
\begin{corollary}{}{}
    A function \( f: D \subseteq \mathbb{R}^n \to \mathbb{R} \) on a convex domain \( D \) is \textbf{strongly convex} with parameter \( \mu > 0 \) iff the function \( g(\mathbf{x}) := \left( f(\mathbf{x}) - \frac{\mu}{2} {\Vert \mathbf{x} \Vert}^2 \right) \) is convex on \( D \).
\end{corollary}
