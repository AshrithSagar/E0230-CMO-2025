...
\begin{definition}{Global minimum}{global-minimum}
    The point \( \xstar \in \R^d \) is a \textbf{global minimum} of the function \( f: \calD \subseteq \R^d \to \R \) if
    \vspace{-0.5em}
    \begin{equation*}
...
\begin{definition}{Strict global minimum}{}
    The point \( \xstar \in \R^d \) is a \textbf{strict global minimum} of the function \( f: \calD \subseteq \R^d \to \R \) if
    \vspace{-0.5em}
    \begin{equation*}
...
\begin{definition}{Local minimum}{local-minimum}
...
\begin{proof}
    Since \( f \in \classC^1 \), from Taylor's theorem~\psecref{sec:multivariate-taylor-theorem}, there exists a \( \delta_1 > 0 \) such that
    \begin{equation*}
        \func{f}{\x}
        =
        \func{f}{\xstar}
    ...
    \end{equation*}
    Since \( \xstar \) is a local minimum, there exists a \( \delta_2 > 0 \) by~\pdefref{def:local-minimum} such that
    \begin{equation*}
        \func{f}{\xstar}
        \leq
...
\begin{proof}
    ...
    Since \( f \in \classC^2 \), from Taylor's theorem~\psecref{sec:multivariate-taylor-theorem}, there exists a \( \delta_1 > 0 \) such that
    \begin{equation*}
        \func{f}{\x}
        =
        \func{f}{\xstar}
    ...
    \end{equation*}
    This is possible only if \( \qf{\p}{\hess{f}{\xstar}} \geq 0, \; \forall \p \in \R^d \setminus \set{\zero} \implies \hess{f}{\xstar} \succeq \zero \).
\end{proof}
...
\begin{definition}{Convex set}{}
    A set \( \calS \subseteq \R^n \) is \textbf{convex} if
    \vspace{-1em}
    \begin{equation*}
...
\begin{definition}{Convex function}{}
    A function \( f: \calD \subseteq \R^n \to \R \) is \textbf{convex} on a convex set \( \calD \) if
    \vspace{-0.5em}
    \begin{equation*}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \quad \boldsymbol{\xi} = (1 - t) \x + t \y, \; t \in (0, 1)
    \end{equation*}
    for some \( t \in (0, 1) \).
    Therefore, we have
    \begin{equation*}
        {(\y - \x)}^\top \hess{f}{\boldsymbol{\xi}} (\y - \x)
        \geq 0,
        \quad \forall \x, \y \in \calD
    \end{equation*}
    which implies \( \hess{f}{\x} \succeq \zero, \; \forall \x \in \calD \).
    (Note: \( \boldsymbol{\xi} \in \calD \) since \( \calD \) is convex.)
...
\section{Strongly convex quadratic minimisation problem}
...
From Taylor's theorem~\psecref{sec:multivariate-taylor-theorem}, we have
\begin{equation*}
    \grad{f}{\y}
    =
    \grad{f}{\x}
...
Note that, for strongly convex quadratic functions~\psecref{sec:strongly-convex-quadratic-minimisation-problem}, from Taylor's theorem~\psecref{sec:multivariate-taylor-theorem}, we have
\begin{align*}
    \because
    \grad{f}{\y}
    & =
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lll}
        ...
        \(
            ...
            \frac{
                \outp
                {\pbrac{\boldsymbol{\delta}_k - \A_k \, \boldsymbol{\gamma}_k}}
                {\pbrac{\boldsymbol{\delta}_k - \A_k \, \boldsymbol{\gamma}_k}}
            }
            {
                \dotp
        ...
        \(
            ...
            \frac{
                \outp
                {\pbrac{\boldsymbol{\gamma}_k - \B_k \, \boldsymbol{\delta}_k}}
                {\pbrac{\boldsymbol{\gamma}_k - \B_k \, \boldsymbol{\delta}_k}}
            }
            {
                \dotp
        ...
        \(
            ...
            +
            \frac
            {\outp{\boldsymbol{\delta}_k}{\boldsymbol{\delta}_k}}
            {\dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}}
            -
            \frac
            {\A_k \outp{\boldsymbol{\gamma}_k}{\boldsymbol{\gamma}_k} \A_k}
            {\qf{\boldsymbol{\gamma}_k}{\A_k}}
        \)
        &
        \(
            \displaystyle
            \pbrac{
                ...
                -
                \frac
                {\outp{\boldsymbol{\gamma}_k}{\boldsymbol{\delta}_k}}
                {\dotp{\boldsymbol{\gamma}_k}{\boldsymbol{\delta}_k}}
            }
            \B_k
            \pbrac{
                \I
                -
                \frac
                {\outp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}}
                {\dotp{\boldsymbol{\gamma}_k}{\boldsymbol{\delta}_k}}
            }
            +
            \frac
            {\outp{\boldsymbol{\gamma}_k}{\boldsymbol{\gamma}_k}}
            {\dotp{\boldsymbol{\gamma}_k}{\boldsymbol{\delta}_k}}
        \)
        \\
        \midrule
        ...
        \(
            ...
            -
            \frac
            {\outp{\pbrac{\B_k \boldsymbol{\delta}_k}}{\pbrac{\B_k \boldsymbol{\delta}_k}}}
            {\qf{\boldsymbol{\delta}_k}{\B_k}}
        \)
        \\
        \midrule
        Broyden family
...
\begin{align*}
    ...
    \A_{k+1} \, \boldsymbol{\gamma}_k
    & =
    \A_k \, \boldsymbol{\gamma}_k
    + \beta \, \outp{\v}{\v} \, \boldsymbol{\gamma}_k
    \\
    \implies
    \boldsymbol{\delta}_k
    - \A_k \, \boldsymbol{\gamma}_k
    & =
    \beta \pbrac{\dotp{\v}{\boldsymbol{\gamma}_k}} \, \v
\end{align*}
...
\begin{align*}
    ...
    - \A_k \, \boldsymbol{\gamma}_k,
    \qquad
    \beta \pbrac{\dotp{\v}{\boldsymbol{\gamma}_k}}
    =
    1
    \\
    \implies
    ...
    \boxed{
        ...
        =
        \frac{1}{
            \dotp{\pbrac{\boldsymbol{\delta}_k
            - \A_k \, \boldsymbol{\gamma}_k}}
            {\boldsymbol{\gamma}_k}
        }
    }
...
\begin{equation*}
    ...
    \geq
    \rho
    \, \norm{\boldsymbol{\delta}_k - \A_k \, \boldsymbol{\gamma}_k}
    \, \norm{\boldsymbol{\gamma}_k},
    \quad
    \text{for some small }
    \rho \in (0, 1)
\end{equation*}
...
\begin{theorem}{}{}
    ...
    the iterates generated in the SR1 update \( \set{\x^i}_{i = 0}^k \) satisfy
    \vspace{-0.5em}
    \begin{equation*}
        \A_k \, \boldsymbol{\gamma}_i
        =
        \boldsymbol{\delta}_i,
        \quad \forall i \in \set{0, 1, \ldots, k - 1}
    \end{equation*}
...
\begin{proof}
    ...
    \paragraph{Base case:}
    For \( k = 1 \), the statement
    \(
        \A_1 \, \boldsymbol{\gamma}_0
        =
        \boldsymbol{\delta}_0
    \)
    ...
    Assume that the statement holds true for \( k = m \), i.e.,
    \begin{equation*}
        \A_m \, \boldsymbol{\gamma}_i
        =
        \boldsymbol{\delta}_i,
        \quad \forall i \in \set{0, 1, \ldots, m - 1}
    \end{equation*}
    We need to show that the statement holds true for \( k = m + 1 \), i.e.,
    \begin{equation*}
        \A_{m + 1} \, \boldsymbol{\gamma}_i
        =
        \boldsymbol{\delta}_i,
        \quad \forall i \in \set{0, 1, \ldots, m}
    \end{equation*}
    ...
    \begin{align*}
        \implies
        \dotp
        {\pbrac{\boldsymbol{\delta}_m - \A_m \, \boldsymbol{\gamma}_m}}
        {\boldsymbol{\gamma}_i}
        & =
        \dotp
        {\boldsymbol{\delta}_m}
        ...
        \dotp
        {\boldsymbol{\delta}_m}
        {\pbrac{\Q \boldsymbol{\delta}_i}}
        -
        \dotp
        {\pbrac{\Q \boldsymbol{\delta}_m}}
        {\boldsymbol{\delta}_i}
        =
        \cancel{
            \qf
    ...
    Thus, by mathematical induction, the result holds true for all \( k \geq 1 \).
\end{proof}
...
\begin{align*}
    ...
    & =
    \A_k \, \boldsymbol{\gamma}_k
    + \beta_1 \, \outp{\u}{\u} \, \boldsymbol{\gamma}_k
    + \beta_2 \, \outp{\v}{\v} \, \boldsymbol{\gamma}_k
    \\
    \implies
    \boldsymbol{\delta}_k
    - \A_k \, \boldsymbol{\gamma}_k
    & =
    \beta_1 \pbrac{\dotp{\u}{\boldsymbol{\gamma}_k}} \, \u
    +
    \beta_2 \pbrac{\dotp{\v}{\boldsymbol{\gamma}_k}} \, \v
\end{align*}
...
\begin{align*}
    ...
    \A_k \, \boldsymbol{\gamma}_k,
    \qquad
    \beta_1 \pbrac{\dotp{\u}{\boldsymbol{\gamma}_k}}
    =
    1,
    \quad
    \beta_2 \pbrac{\dotp{\v}{\boldsymbol{\gamma}_k}}
    =
    -1
    \\
    \implies
    ...
    \boxed{
        ...
        \beta_1
        =
        \frac{1}{\dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}},
        \quad
        \beta_2
        =
        -\frac{1}{\qf{\boldsymbol{\gamma}_k}{\A_k}}
    }
\end{align*}
...
The DFP update guarantees positive definiteness of \( \A_{k+1} \) if \( \A_k \succ \zero \) and \( \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k} > 0 \).

The condition \( \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k} > 0 \) is known as the \textit{curvature condition}, and is satisfied if \( f \) is strongly convex and \( \alpha_k \) is chosen using exact line search or an inexact line search that satisfies the Wolfe conditions.

\begin{theorem}{DFP positive definiteness}{}
    If \( \A_0 \) is symmetric positive-definite and \( \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k} > 0, \; \forall k \), then the DFP update preserves positive definiteness, i.e., if \( \A_k \succ \zero \) and \( \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k} > 0 \), then \( \A_{k+1} \succ \zero \).
\end{theorem}
...
\begin{proof}
    ...
    Setting \( \a = \dotp{\mathbf{L}_k}{\z}, \; \b = \dotp{\mathbf{L}_k}{\boldsymbol{\gamma}_k} \), where \( \z \in \R^d \setminus \set{\zero} \), we have
    \begin{align*}
        \implies
        \qf{\z}{\A_{k+1}}
        & =
        ...
        +
        \frac
        {\pbrac{\dotp{\boldsymbol{\delta}_k}{\z}}^2}
        {\dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}}
        -
        \qf{\z}{
            \frac
            {\outp{\mathbf{L}_k}{\mathbf{L}_k} \outp{\boldsymbol{\gamma}_k}{\boldsymbol{\gamma}_k} \outp{\mathbf{L}_k}{\mathbf{L}_k}}
            {\qf{\boldsymbol{\gamma}_k}{\outp{\mathbf{L}_k}{\mathbf{L}_k}}}
        }
        =
        \frac
        {\pbrac{\dotp{\boldsymbol{\delta}_k}{\z}}^2}
    ...
    Since \( \mathbf{L}_k^\top \) is invertible, and thereby it's nullspace is \( \set{\zero} \), which implies \( \b = \zero \iff \boldsymbol{\gamma}_k = \zero \).\\
    But \( \boldsymbol{\gamma}_k = \zero \implies \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k} = 0 \), a contradiction.
    Thus, \( \boldsymbol{\gamma}_k \neq \zero \), and thereby \( \b \neq \zero \).
    ...
    \begin{align*}
        ...
        =
        \lambda
        \dotp{\mathbf{L}_k}{\boldsymbol{\gamma}_k}
        \iff
        \z
        =
        \lambda \boldsymbol{\gamma}_k
        \\
        \text{Now, }
        \because
        \z
        ...
        \quad
        \because
        \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k} > 0,
        \; \lambda \neq 0
        \\
        \implies
        \qf{\z}{\A_{k+1}}
        ...
        > 0,
        \quad \forall \z \in \spanset{\boldsymbol{\gamma}_k} \setminus \set{\zero}
    \end{align*}

    In the strict inequality case, we have \( \z \notin \spanset{\boldsymbol{\gamma}_k} \), thereby
    \begin{equation*}
        \qf{\z}{\A_{k+1}}
        =
        \overbrace{
        ...
        > 0,
        \quad \forall \z \notin \spanset{\boldsymbol{\gamma}_k}
    \end{equation*}
...
\begin{theorem}{}{}
    For strongly convex quadratic functions~\psecref{sec:strongly-convex-quadratic-minimisation-problem},
    the directions generated in the DFP update \( \set{\u^i}_{i = 0}^k \) satisfy
    \vspace{-1em}
    \begin{equation*}
        \qf{\u^k}{\Q}[\u^i]
        = 0,
...
\begin{proof}
    ...
    \begin{align*}
        ...
        =
        \frac{1}{\alpha_0} \boldsymbol{\delta}_0
        \\
        \implies
        \qf{\u^1}{\Q}[\u^0]
        & =
        -\frac{1}{\alpha_0} \qf{\grad{f}{\x^1}}{\A_1 \Q}[\boldsymbol{\delta}_0]
        =
        -\frac{1}{\alpha_0} \qf{\grad{f}{\x^1}}{\A_1}[\boldsymbol{\gamma}_0]
        \\ & =
        -\frac{1}{\alpha_0} \dotp{\grad{f}{\x^1}}{\boldsymbol{\delta}_0}
        =
        -\dotp{\grad{f}{\x^1}}{\u^0}
        = 0
    \end{align*}
    where we have used the secant equation~\peqref{eq:secant-equation-quadratic} with \( k = 0 \), the Quasi-Newton condition~\peqref{eq:quasi-newton-condition} for \( k = 0 \), and the expanding subspace theorem~\pthmref{thm:expanding-subspace-theorem} holds since the directions \( \set{\u^i}_{i = 0}^1 \) are mutually \( \Q \)--conjugate.

    \begin{align*}
        \text{Now, }
        \dotp{\boldsymbol{\delta}_1}{\boldsymbol{\gamma}_0}
        & =
        \qf{\boldsymbol{\delta}_1}{\Q}[\boldsymbol{\delta}_0]
        =
        \qf{\pbrac{\alpha_1 \u^1}}{\Q}[\pbrac{\alpha_0 \u^0}]
        =
        \alpha_0 \alpha_1
        \qf{\u^1}{\Q}[\u^0]
        = 0
        \\
        \text{And, }
        \qf{\boldsymbol{\gamma}_1}{\A_1}[\boldsymbol{\gamma}_0]
        & =
        \dotp{\boldsymbol{\gamma}_1}{\boldsymbol{\delta}_0}
        =
        \dotp{\pbrac{\Q \u^1}}{\pbrac{\alpha_0 \u^0}}
        =
        \alpha_0
        \qf{\u^1}{\Q}[\u^0]
        = 0
        \\
        \implies
        \A_2 \, \boldsymbol{\gamma}_0
        & =
        \pbrac{
            \A_1
            +
            \frac
            {\outp{\boldsymbol{\delta}_1}{\boldsymbol{\delta}_1}}
            {\dotp{\boldsymbol{\delta}_1}{\boldsymbol{\gamma}_1}}
            -
            \frac
            {\A_1 \outp{\boldsymbol{\gamma}_1}{\boldsymbol{\gamma}_1} \A_1}
            {\qf{\boldsymbol{\gamma}_1}{\A_1}}
        } \, \boldsymbol{\gamma}_0
        =
        \A_1 \, \boldsymbol{\gamma}_0
        =
        \boldsymbol{\delta}_0
    \end{align*}
    where we have used the secant equation~\peqref{eq:secant-equation-quadratic} and the Quasi-Newton condition~\peqref{eq:quasi-newton-condition} for \( k = 0 \).

    Also, from the Quasi-Newton condition~\peqref{eq:quasi-newton-condition} for \( k = 1 \), we have
    \(
        \A_{2} \, \boldsymbol{\gamma}_1
        =
        \boldsymbol{\delta}_1
    \).
    ...
    \begin{align*}
        ...
        =
        \frac{1}{\alpha_i} \boldsymbol{\delta}_i
        \\
        \implies
        \qf{\u^m}{\Q}[\u^i]
        & =
        -\frac{1}{\alpha_i} \qf{\grad{f}{\x^m}}{\A_m \Q}[\boldsymbol{\delta}_i]
        =
        -\frac{1}{\alpha_i} \qf{\grad{f}{\x^m}}{\A_m}[\boldsymbol{\gamma}_i]
        \\ & =
        -\frac{1}{\alpha_i} \dotp{\grad{f}{\x^m}}{\boldsymbol{\delta}_i}
        =
        -\dotp{\grad{f}{\x^m}}{\u^i}
        = 0,
        \quad \forall i \in \set{0, 1, \ldots, m - 1}
    \end{align*}
    where we have used the secant equation~\peqref{eq:secant-equation-quadratic} with \( k = i \), the induction hypothesis, and the expanding subspace theorem~\pthmref{thm:expanding-subspace-theorem} holds since the directions \( \set{\u^i}_{i = 0}^m \) are mutually \( \Q \)--conjugate.

    Similarly, for any \( i \in \set{0, 1, \ldots, m - 1} \), we have
    \begin{align*}
        \implies
        \dotp{\boldsymbol{\delta}_m}{\boldsymbol{\gamma}_i}
        & =
        \qf{\boldsymbol{\delta}_m}{\Q}[\boldsymbol{\delta}_i]
        =
        ...
        \\
        \implies
        \qf{\boldsymbol{\gamma}_m}{\A_m}[\boldsymbol{\gamma}_i]
        & =
        \dotp{\boldsymbol{\gamma}_m}{\boldsymbol{\delta}_i}
        =
    ...
    \begin{equation*}
        ...
        \pbrac{
            ...
            +
            \frac
            {\outp{\boldsymbol{\delta}_m}{\boldsymbol{\delta}_m}}
            {\dotp{\boldsymbol{\delta}_m}{\boldsymbol{\gamma}_m}}
            -
            \frac
            {\A_m \outp{\boldsymbol{\gamma}_m}{\boldsymbol{\gamma}_m} \A_m}
            {\qf{\boldsymbol{\gamma}_m}{\A_m}}
        } \, \boldsymbol{\gamma}_i
        =
        \A_m \, \boldsymbol{\gamma}_i
        =
...
\begin{equation*}
    ...
    =
    \A_k
    + \beta_1 \, \outp{\boldsymbol{\delta}_k}{\boldsymbol{\delta}_k}
    + \beta_2 \, \pbrac{
        \boldsymbol{\delta}_k \dotp{\boldsymbol{\gamma}_k}{\A_k}
        + \outp{\A_k \, \boldsymbol{\gamma}_k}{\boldsymbol{\delta}_k}
    }
    + \beta_3 \, \A_k \outp{\boldsymbol{\gamma}_k}{\boldsymbol{\gamma}_k} \A_k,
...
\begin{align*}
    ...
    - \A_k \, \boldsymbol{\gamma}_k
    & =
    \beta_1 \outp{\boldsymbol{\delta}_k}{\boldsymbol{\delta}_k} \boldsymbol{\gamma}_k
    + \beta_2 \pbrac{
        \boldsymbol{\delta}_k \dotp{\boldsymbol{\gamma}_k}{\A_k} \boldsymbol{\gamma}_k
        + \A_k \, \boldsymbol{\gamma}_k \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}
    }
    + \beta_3 \, \A_k \outp{\boldsymbol{\gamma}_k}{\boldsymbol{\gamma}_k} \A_k \boldsymbol{\gamma}_k
    \\ & =
    \pbrac{
        \beta_1 \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}
        + \beta_2 \qf{\boldsymbol{\gamma}_k}{\A_k}
    } \boldsymbol{\delta}_k
    + \pbrac{
        \beta_2 \dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}
        + \beta_3 \qf{\boldsymbol{\gamma}_k}{\A_k}
    } \A_k \, \boldsymbol{\gamma}_k
\end{align*}
...
\begin{equation*}
    \boxed{
        ...
        =
        \frac{1}{\dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}}
        \pbrac{1 + \phi \, \frac{\qf{\boldsymbol{\gamma}_k}{\A_k}}{\dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}}},
        \quad
        \beta_2
        =
        -\frac{\phi}{\dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}},
        \quad
        \beta_3
        =
        -\frac{\pbrac{1 - \phi}}{\qf{\boldsymbol{\gamma}_k}{\A_k}},
        \quad
        \; \phi \in [0, 1]
    }
\end{equation*}
...
\begin{equation*}
    ...
    \boxed{
        ...
        \v
        =
        \sqrt{\qf{\boldsymbol{\gamma}_k}{\A_k}}
        \bbrac{
            \frac
            {\boldsymbol{\delta}_k}
            {\dotp{\boldsymbol{\delta}_k}{\boldsymbol{\gamma}_k}}
