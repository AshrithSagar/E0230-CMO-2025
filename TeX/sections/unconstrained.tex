...
\begin{lemma}{Sufficient decrease lemma (Armijo condition)}{}
    Suppose \( f \in \mathcal{C}^{1} \) is bounded below and \( \nabla f \in \mathcal{C}^{1}_{L} \), then for any \( \mathbf{x} \in \mathbb{R}^n \) and any step size \( \alpha \in \linterval[scaled]{0}{\frac{1}{L}} \), the gradient descent update
    \begin{equation*}
        \mathbf{x}^+ = \mathbf{x} - \alpha \nabla f(\mathbf{x})
    \end{equation*}
    yields a sufficient decrease in the function value:
    \begin{equation*}
