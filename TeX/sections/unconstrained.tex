...
\begin{algorithm}[H]
    ...
    \While{\( k < d \) \emph{ AND } \( \xk \) is not optimal}{
        Choose step length \( \alpha_k \) by exact line search along direction \( \mathbf{u}_k \) as
        \vspace{-0.5em}
        \begin{equation*}
            \alpha_k
            =
...
\begin{algorithm}[H]
    ...
    \( \mathbf{u}_0 \leftarrow -\grad{f}{\xzero} \)\;
    ...
    \While{\( k < d \) \emph{ AND } \( \xk \) is not optimal}{
        ...
        \begin{equation*}
            ...
            + \beta_k \mathbf{u}_k
        \end{equation*}
        such that \( \mathbf{u}_{k+1} \) is \( \Q \)--conjugate to \( \mathbf{u}_k \), thereby
        \vspace{-0.5em}
        \begin{equation*}
            \beta_k
            =
            \frac{\qf{\grad{f}{\xkp}}{\Q}[\mathbf{u}_k]}{\qf{\mathbf{u}_k}{\Q}}
        \end{equation*}
...
\begin{theorem}{Mutual \( \Q \)--conjugacy of directions in conjugate gradient method}{}
    The directions \( \set{\mathbf{u}_k}_{k = 0}^{d - 1} \) generated by the conjugate gradient method are mutually \( \Q \)--conjugate.

    For all \( k \in \set{1, 2, \ldots, d - 1} \), we have
    \vspace{-0.5em}
    \begin{align*}
        \spanset{\mathbf{u}_0, \ldots, \mathbf{u}_k}
        & =
        \spanset{\grad{f}{\x_0}, \ldots, \grad{f}{\x_k}}
        =
        \spanset{\grad{f}{\x_0}, \Q \, \grad{f}{\x_0}, \ldots, \Q^k \, \grad{f}{\x_0}}
        \\
        \qf{\mathbf{u}_k}{\Q}[\mathbf{u}_i]
        & = 0,
