...
\begin{definition}{Strict global minimum}{}
    The point \( \xstar \in \R^d \) is a \textbf{strict global minimum} of the function \( f: \D \subseteq \R^d \to \R \) if
    \vspace{-0.5em}
    \begin{equation*}
        \func{f}{\xstar} < \func{f}{\x},
        \quad \forall \x \in \D \setminus \set{\xstar}
    \end{equation*}
\end{definition}
...
\begin{corollary}{First-order variational inequality condition}{first-order-variational-inequality-condition}
    For a function \( f \in \C^1 \), if \( \xstar \) is a local minimum of \( f \), then there exists a \( \delta > 0 \) such that
    \vspace{-0.5em}
    \begin{equation*}
...
\begin{proof}
    Since \( f \in \C^1 \), from Taylor's theorem~\psecref{sec:multivariate-taylor-theorem}, there exists a \( \delta_1 > 0 \) such that
    \begin{equation*}
        \func{f}{\x}
        =
        \func{f}{\xstar}
...
\begin{proof}
    \( \because \C^2 \subseteq \C^1 \), by the first-order necessary condition for optimality~\pthmref{thm:first-order-necessary-condition-for-a-local-minimum}, \( \grad{f}{\xstar} = \zero \).

    Since \( f \in \C^{2} \), from Taylor's theorem~\psecref{sec:multivariate-taylor-theorem}, there exists a \( \delta_1 > 0 \) such that
    \begin{equation*}
        \func{f}{\x}
        =
        \func{f}{\xstar}
...
\begin{definition}{Convex set}{}
    A set \( \mathcal{S} \subseteq \R^n \) is \textbf{convex} if
    \vspace{-1em}
    \begin{equation*}
        \lambda \x + (1 - \lambda) \y \in \mathcal{S},
        \quad \forall \x, \y \in \mathcal{S},
        \; \lambda \in [0, 1]
    \end{equation*}
\end{definition}
...
\begin{corollary}{}{}
    A function \( f: \D \subseteq \R^n \to \R \) on a convex domain \( \D \) is strongly convex with parameter \( \mu > 0 \) iff the function \( \func{g}{\x} := \pbrac{\func{f}{\x} - \frac{\mu}{2} \norm{\x}^2} \) is convex on \( \D \).
\end{corollary}
...
\begin{equation*}
    ...
    \boxed{
        ...
        \A_k^{\text{DFP}}
        +
        \phi \, \boldsymbol{\delta}_k \dotp{\boldsymbol{\gamma}_k}{\A_k}
    }
\end{equation*}
