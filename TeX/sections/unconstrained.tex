...
\begin{theorem}{Conjugate gradient theorem:\\ Mutual \( \Q \)--conjugacy of directions in conjugate gradient method}{}
...
\begin{algorithm}[H]\label{alg:practical-conjugate-gradient-method}
    ...
    \While{\( k < d \) \emph{ AND } \( \x^k \) is not optimal}{
        ...
        Find the new gradient by using:
        \vspace{-0.5em}
        \begin{equation*}
            \grad{f}{\x^{k+1}}
            =
...
\begin{proof}
    The only differences between the two algorithms are in the equations used to compute \( \alpha_k \) and \( \beta_k \), and the way to obtain \( \grad{f}{\x^{k+1}} \).
