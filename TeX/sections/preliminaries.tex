...
\paragraph{\( L_1 \) norm (Maximum absolute column sum norm)}
...
\paragraph{\( L_2 \) norm (Spectral norm)}
...
\paragraph{\( L_{\infty} \) norm (Maximum absolute row sum norm)}
...
\subsubsection{Frobenius norm (Euclidean norm)}
...
\begin{itemize}
    ...
    \item \( \rho(\mathbf{A}) \leq \norm{\mathbf{A}^k}^{\frac{1}{k}}, \quad \forall k \in \N, \; \forall \text{ consistent matrix norms } \norm{\,\cdot\,} \)
