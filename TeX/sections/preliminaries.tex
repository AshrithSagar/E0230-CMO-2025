...
A sequence of real numbers \( {\{ a_n \}}_{n=1}^{\infty} \) is said to \textbf{converge} to a limit \( L \in \mathbb{R} \) if
\begin{equation*}
    a_n \to L
    \quad \iff \quad
    ...
    \text{s.t.} \
    \forall n > N \implies \vert a_n - L \vert < \epsilon
\end{equation*}
...
\begin{equation*}
    ...
    \exists N \in \mathbb{N}, \
    \text{s.t.} \
    \forall m, n > N \implies \vert a_n - a_m \vert < \epsilon
\end{equation*}

In \( \mathbb{R} \), every Cauchy sequence is convergent, and every convergent sequence is Cauchy, since \( \mathbb{R} \) is complete.

\chapter{Calculus}

\section{Limits}

(\( \epsilon \)--\( \delta \)) definition of \textbf{limit} of a function \( f: D \subseteq \mathbb{R}^d \to \mathbb{R} \) at a limit point \( \mathbf{x}_0 \) of \( D \):
\begin{equation*}
...
Sequence definition of \textbf{limit} of a function \( f: D \subseteq \mathbb{R}^d \to \mathbb{R} \) at a limit point \( \mathbf{x}_0 \) of \( D \):
\begin{equation*}
    \lim_{\mathbf{x} \to \mathbf{x}_0} f(\mathbf{x}) = L
    \; \iff \;
    \lim_{n \to \infty} \vert f(\mathbf{x}_n) - L \vert = 0
    \; \iff \;
...
\begin{itemize}
    ...
    \item If \( f \in \mathcal{C}^{1}_L \), then
        ...
        \begin{proof}
            Follows from \( f(\mathbf{x}) \pm \frac{L}{2} {\Vert \mathbf{x} \Vert}^2 \) being convex functions, given \( f \in \mathcal{C}^{1}_L \).
        \end{proof}
\end{itemize}
...
The \textbf{outer product} of two vectors \( \mathbf{x} \in \mathbb{R}^m \) and \( \mathbf{y} \in \mathbb{R}^n \) is defined as
\begin{equation*}
...
\begin{itemize}
    ...
    \item \( \mathbf{x} \mathbf{x}^\top \in \mathbf{S}_d^+, \; \forall \mathbf{x} \in \mathbb{R}^d \).

    \item \( \operatorname{spec}(\mathbf{x} \mathbf{x}^\top) = \{ {\Vert \mathbf{x} \Vert}_2^2, 0^{(d-1)} \} \).
        Eigenvectors: \( \{ \mathbf{x} \} \cup \underbrace{ \{ \mathbf{v} \mid \mathbf{v}^\top \mathbf{x} = 0 \} }_{\mathbf{x}^\perp} \).

        Eigenvalues of \( \mathbf{x} \mathbf{x}^\top \) are \( \|\mathbf{x}\|_2^2 \) and \( 0 \) (with multiplicity \( d-1 \)).
        The eigenvector corresponding to \( \|\mathbf{x}\|_2^2 \) is \( \mathbf{x} \); the eigenspace corresponding to \( 0 \) is the orthogonal complement \( \mathbf{x}^\perp \).
\end{itemize}
...
\begin{itemize}
    \item \( \operatorname{spec}(\mathbf{A}) \subseteq \mathbb{R}^d \), for \( \mathbf{A} \in \mathbf{S}_{d} \).

    \item \( \mathbf{v}_i^\top \mathbf{v}_j = 0, \; \text{if } \lambda_i \neq \lambda_j \), for \( \mathbf{A} \in \mathbf{S}_{d} \).
