...
\begin{equation*}
    ...
    =
    \sqrt{\operatorname{trace}(\A^\top \A)}
    =
...
\begin{itemize}
    \item \( \operatorname{rank} (\x \y^\top) = 1, \; \forall \x \in \R^m \setminus \set{\zero}, \; \forall \y \in \R^n \setminus \set{\zero} \).

    \item \( \x \x^\top \in \mathbf{S}_d^+, \; \forall \x \in \R^d \).

    \item \( \operatorname{spec}(\x \x^\top) = \set{\norm{\x}_2^2, 0^{(d-1)}} \).
...
\begin{itemize}
    ...
    \item \( \A \in \SD \implies \A^{-1} \in \SD, \text{ if } \A \text{ is invertible} \).
...
\begin{itemize}
    \item \( \operatorname{trace}(\A) = \sum_{i = 1}^{d} a_{ii} = \sum_{i = 1}^{d} \lambda_i \).

    \item \( \det(\A) = \prod_{i = 1}^{d} \lambda_i \).
...
\begin{theorem}{Spectral Decomposition Theorem}{}
    ...
    \end{equation*}
    where \( \mathbf{V} = [\v_1, \v_2, \ldots, \v_d] \in \R^{d \times d} \) is an orthogonal matrix (i.e., \( \dotp{\V}{\V} = \I \)) and \( \boldsymbol{\Lambda} = \operatorname{diag}(\lambda_1, \lambda_2, \ldots, \lambda_d) \in \R^{d \times d} \) is a diagonal matrix containing the eigenvalues of \( \A \).
\end{theorem}
...
\begin{proof}
    ...
    This gives that \( \mathbf{V} \) is an orthogonal matrix \( \implies \dotp{\V}{\V} = \I \implies \mathbf{V}^{-1} = \mathbf{V}^\top \).

    Now, with \( \boldsymbol{\Lambda} = \operatorname{diag}(\lambda_1, \lambda_2, \ldots, \lambda_d) \) and \( \mathbf{V} = [\v_1, \v_2, \ldots, \v_d] \), we have
    \begin{equation*}
        \A \mathbf{V}
        =
