...
A function \( f: \mathbb{R}^d \to \mathbb{R} \) is said to be of \textbf{class} \( C^k \) if all its partial derivatives up to order \( k \in \mathbb{N} \) exist and are continuous.
...
\begin{itemize}
    ...
    \item (Schwarz's theorem) If \( f \in C^2 \), then \( \nabla^2 f(\mathbf{x}) \in \mathbf{S}_d\), i.e., \( \frac{\partial^2 f}{\partial x_i \partial x_j} = \frac{\partial^2 f}{\partial x_j \partial x_i}, \ \forall i, j = 1, 2, \ldots, d \).

    \item If \( f, g \in C^k \), then \( f + g, f \cdot g \in C^k \).

    \item If \( f \in C^k \) and \( g: \mathbb{R} \to \mathbb{R} \) is \( k \)-times differentiable, then \( g \circ f \in C^k \).
