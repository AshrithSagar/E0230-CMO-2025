...
A \textbf{normed vector space} \( (V, \| \cdot \|) \) is a vector space \( V \) over a field \( \mathbb{F} \) (usually \( \R \) or \( \mathbb{C} \)) equipped with a function \( \| \cdot \|: V \to \R \) that satisfies the following properties for all \( u, v \in V \) and \( a \in \mathbb{F} \):
\begin{itemize}
...
\begin{itemize}
    ...
    \item \( \eta(\mathbf{x}) = 0 \iff \mathbf{x} = \zero \)
    \item \( \eta(s\mathbf{x}) = |s| \; \eta(\mathbf{x}) \), \quad \( s \in \R \)
    \item \( \eta(\mathbf{x} + \mathbf{y}) \leq \eta(\mathbf{x}) + \eta(\mathbf{y}) \)
\end{itemize}

\subsection{\texorpdfstring{\( L_p \) norm}{Lp norm}}
\begin{equation*}
    \normbf{x}_p = {\left( \sum_{i=1}^{d} {\vert x_i \vert}^p \right)}^{\frac{1}{p}},
    \quad p \geq 1
\end{equation*}
...
A \textbf{neighborhood} of a point \( \mathbf{x} \in \R^d \) is an open ball centered at \( \mathbf{x} \) with radius \( r > 0 \), defined as
\begin{equation*}
    B_{r}(\mathbf{x}) = \left\{ \mathbf{z} \in \R^d \;\big|\; \normbf{z - x} < r \right\}, \quad r > 0, \ \mathbf{x} \in \R^d
\end{equation*}

\section{Interior point}

A point \( \mathbf{x} \in S \) is an \textbf{interior point} of set \( S \subseteq \R^d \) if there exists a neighborhood of \( \mathbf{x} \) that is entirely contained within \( S \), i.e., there exists an \( r > 0 \) such that \( B_{r}(\mathbf{x}) \subseteq S \).

\section{Limit point}

A point \( \mathbf{x} \in \R^d \) is a \textbf{limit point} of set \( S \subseteq \R^d \) if every neighborhood of \( \mathbf{x} \) contains at least one point of \( S \) different from \( \mathbf{x} \) itself, i.e., for every \( r > 0 \), there exists a point \( \mathbf{y} \in S \setminus \{ \mathbf{x} \} \) such that \( \mathbf{y} \in B_{r}(\mathbf{x}) \).
\begin{equation*}
...
A set \( S \subseteq \R^d \) is \textbf{closed} if it contains all its limit points, i.e., if \( \mathbf{x} \) is a limit point of \( S \), then \( \mathbf{x} \in S \).

\section{Bounded set}

A set \( S \subseteq \R^d \) is \textbf{bounded} if there exists \( \mathbf{z} \in \R^d, M \in \R \) such that \( \normbf{z - x} < M, \ \forall \mathbf{x} \in S \).
...
(\( \epsilon \)--\( \delta \)) definition of \textbf{limit} of a function \( f: D \subseteq \R^d \to \R \) at a limit point \( \mathbf{x}_0 \) of \( D \):
\begin{equation*}
    ...
    \exists \delta > 0, \
    \text{s.t.} \
    0 < \norm{\mathbf{x} - \mathbf{x}_0} < \delta, \,
    \mathbf{x} \in D
    \implies
    \abs{f(\mathbf{x}) - L} < \epsilon
\end{equation*}
...
\begin{equation*}
    ...
    \exists \delta > 0, \
    \text{s.t.} \
    \mathbf{x} \in D \cap B_{\delta}(\mathbf{x}_0) \setminus \{ \mathbf{x}_0 \}
    \implies
    f(\mathbf{x}) \in B_{\epsilon}(L)
\end{equation*}

Sequence definition of \textbf{limit} of a function \( f: D \subseteq \R^d \to \R \) at a limit point \( \mathbf{x}_0 \) of \( D \):
\begin{equation*}
    \lim_{\mathbf{x} \to \mathbf{x}_0} f(\mathbf{x}) = L
    \; \iff \;
    \lim_{n \to \infty} \abs{f(\mathbf{x}_n) - L} = 0
    \; \iff \;
    \forall {\{ \mathbf{x}_n \}}_{n=1}^{\infty} \subseteq D \setminus \{ \mathbf{x}_0 \},
    \ \mathbf{x}_n \to \mathbf{x}_0
    \implies
    f(\mathbf{x}_n) \to L
\end{equation*}
...
A function \( f: D \subseteq \R^d \to \R \) is \textbf{continuous} at a point \( \mathbf{x}_0 \in D \) iff
\begin{equation*}
...
The \textbf{gradient} of a function \( f: \R^d \to \R \) at a point \( \mathbf{x} = {\left(x_1, x_2, \ldots, x_d\right)}^{\top} \in \R^d \) is defined as
\begin{equation*}
...
The \textbf{Hessian} of a function \( f: \R^d \to \R \) at a point \( \mathbf{x} = {\left(x_1, x_2, \ldots, x_d\right)}^{\top} \in \R^d \) is defined as
\begin{equation*}
    \nabla^2 f(\mathbf{x}) =
    \begin{bmatrix}
...
If a function \( f: S \to \R \) is continuous on a set \( S \subseteq \R^d \) that is closed and bounded, then \( f \) attains its maximum and minimum values on \( S \), i.e., there exist points \( \mathbf{x}_{\min}, \mathbf{x}_{\max} \in S \) such that
\begin{equation*}
...
\begin{equation*}
    ...
    \leq
    L \; \normbf{x - y}_2,
    \quad \forall \mathbf{x}, \mathbf{y} \in \R^d
\end{equation*}
...
\begin{itemize}
    \item If \( f \in \C^{2} \), then \( f \in \C^{1}_L \) iff \( \nabla^2 f(\mathbf{x}) \preceq L \mathbf{I}, \ \forall \mathbf{x} \in \R^d \).

    \item If \( f \in \C^{1}_L \), then
        \begin{equation*}
            \abs{ f(\mathbf{y}) - f(\mathbf{x}) - {\nabla f(\mathbf{x})}^\top (\mathbf{y} - \mathbf{x}) }
            \leq
            \frac{L}{2} \normbf{y - x}^2,
            \quad \forall \mathbf{x}, \mathbf{y} \in \R^d
        \end{equation*}
        ...
        \begin{proof}
            Follows from \( f(\mathbf{x}) \pm \frac{L}{2} \normbf{x}^2 \) being convex functions, given \( f \in \C^{1}_L \).
        \end{proof}
\end{itemize}
...
The \textbf{outer product} of two vectors \( \mathbf{x} \in \R^m \) and \( \mathbf{y} \in \R^n \) is defined as
\begin{equation*}
...
\begin{itemize}
    \item \( \operatorname{rank} (\mathbf{x} \mathbf{y}^\top) = 1 \), provided \( \mathbf{x} \neq \zero, \; \mathbf{y} \neq \zero \).

    \item \( \mathbf{x} \mathbf{x}^\top \in \mathbf{S}_d^+, \; \forall \mathbf{x} \in \R^d \).

    \item \( \operatorname{spec}(\mathbf{x} \mathbf{x}^\top) = \{ \normbf{x}_2^2, 0^{(d-1)} \} \).
        Eigenvectors: \( \{ \mathbf{x} \} \cup \underbrace{ \{ \mathbf{v} \mid \dotpbf{v}{x} = 0 \} }_{\mathbf{x}^\perp} \).

        Eigenvalues of \( \mathbf{x} \mathbf{x}^\top \) are \( \normbf{x}_2^2 \) and \( 0 \) (with multiplicity \( d-1 \)).
        The eigenvector corresponding to \( \normbf{x}_2^2 \) is \( \mathbf{x} \); the eigenspace corresponding to \( 0 \) is the orthogonal complement \( \mathbf{x}^\perp \).
\end{itemize}
...
\begin{equation*}
    ...
    \quad \forall \mathbf{x} \in \R^d \setminus \{ \zero \}
\end{equation*}
...
\begin{itemize}
    \item \( \operatorname{spec}(\mathbf{A}) \subseteq \R^d \), for \( \mathbf{A} \in \SD \).

    \item \( \mathbf{v}_i^\top \mathbf{v}_j = 0, \; \text{if } \lambda_i \neq \lambda_j \), for \( \mathbf{A} \in \SD \).
...
\begin{theorem}{Spectral Decomposition Theorem}{}
    ...
    \end{equation*}
    where \( \mathbf{V} = [\mathbf{v}_1, \mathbf{v}_2, \ldots, \mathbf{v}_d] \in \R^{d \times d} \) is an orthogonal matrix (i.e., \( \dotpbf{V}{V} = \mathbf{I} \)) and \( \boldsymbol{\Lambda} = \operatorname{diag}(\lambda_1, \lambda_2, \ldots, \lambda_d) \in \R^{d \times d} \) is a diagonal matrix containing the eigenvalues of \( \mathbf{A} \).
\end{theorem}
...
\begin{proof}
    ...
    This gives that \( \mathbf{V} \) is an orthogonal matrix \( \implies \dotpbf{V}{V} = \mathbf{I} \implies \mathbf{V}^{-1} = \mathbf{V}^\top \).
...
\begin{itemize}
    \item The identity matrix \( \mathbf{I} \in \R^{d \times d} \) has eigenvalues \( \lambda_i = 1, \; \forall i \) and the corresponding eigenvectors can be chosen as any orthonormal basis of \( \R^d \), commonly taken as the standard basis vectors \( \mathbf{e}_i \in \R^d \).
    ...
    \item \( \mathbf{A} \in \PD, \; \kappa(\mathbf{A}) = 1 \iff \mathbf{A} = c \mathbf{I}, \; c > 0 \).
\end{itemize}
...
\begin{equation*}
    \abs{\dotpbf{x}{y}} \leq \normbf{x}_2 \; \normbf{y}_2, \quad \mathbf{x}, \mathbf{y} \in \R^d
\end{equation*}
...
\begin{corollary}{Kantorovich inequality for positive definite matrices~\citep{Luenberger1984}}{}
    For any \( \mathbf{Q} \in \PD \), with smallest eigenvalue \( \lambda_{\min} \) and largest eigenvalue \( \lambda_{\max} \), and for any \( \mathbf{x} \in \R^d \setminus \{ \zero \} \),
    \begin{equation*}
        \frac{{(\dotpbf{x}{x})}^2}{(\mathbf{x}^{\top} \mathbf{Q} \mathbf{x})(\mathbf{x}^{\top} \mathbf{Q}^{-1} \mathbf{x})}
        \geq
...
\begin{proof}
    ...
    Since the eigenvectors form an orthonormal basis for \( \R^d \), any vector \( \mathbf{x} \in \R^d \) can be expressed as a linear combination of these eigenvectors as \( \mathbf{x} = \sum_{i=1}^{d} c_i \mathbf{v}_i \), where each coefficient \( c_i \) is the projection of \( \mathbf{x} \) onto the eigenvector \( \mathbf{v}_i \), given by \( c_i = \mathbf{v}_i^\top \mathbf{x} \).
    Now,
    \begin{align*}
...
\begin{equation*}
    \half \norm{\nabla f(\mathbf{x})}^2 \geq \mu \left( f(\mathbf{x}) - f^* \right)
