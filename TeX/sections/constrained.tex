...
\begin{itemize}
    ...
    \item \AP The functions \( c_i: \R^d \to \R, \; i \in \calE \cup \calI \) are called the \textbf{""constraint functions""}.
    ...
    \item A point \( \x \in \R^d \) that satisfies all the constraints is called a \textbf{""feasible point""} / \textbf{candidate solution}.

    \item \AP If a point \( \x \in \R^d \) violates any of the constraints, then it is called an \textbf{infeasible point}.
    ...
    \item \AP An \textbf{""active constraint""} is a constraint that is satisfied with equality at a given feasible point.
    ...
    \item \AP A \textbf{""binding constraint""} is a constraint that is satisfied with equality at a given optimal point.
    ...
    \item \AP The \textbf{""active set""} at a feasible point \( \x \) is the set of indices of the active constraints at \( \x \), i.e.,
        \vspace{-0.5em}
        \begin{equation*}
            \func{\calA}{\x}
            \triangleq
...
\begin{definition}{Lagrangian function}{}
    The \textbf{""Lagrangian function""} \( \calL: \R^d \times \R^{m+p} \to \R \) associated with the "constrained optimisation problem" in~\psecref{sec:constrained-optimisation-standard-form} is defined as
    \vspace{-0.5em}
    \begin{equation*}
        ...
        \func{f}{\x}
        -
        \sum_{i \in \calE \cup \calI} \bbrac{\bflambda}_i \, \func{c_i}{\x}
    \end{equation*}
\end{definition}

Alternatively, the Lagrangian function can be written as
\begin{equation*}
...
The variables \( \lambda_i, \; i \in \calI \), and \( \mu_j, \; j \in \calE \), are called the \textbf{Lagrange multipliers} / \textbf{dual variables} associated with the "inequality@inequality constraints" and "equality constraints" respectively.

\section{Regularity conditions}

The \textbf{""regularity conditions""} / \textbf{constraint qualifications} are a set of conditions on the "constraint functions" that ensure the validity of certain optimality conditions for "constrained optimisation problems".

\subsection{Linear independence constraint qualification (LICQ)}

\begin{definition}{Linear independence constraint qualification (LICQ)~\cite{wiki}}{}
    The \textbf{""linear independence constraint qualification""} (LICQ) holds at a "feasible point" \( \x \) if the gradients of the "active constraints" at \( \x \) are linearly independent, i.e., the gradients of the active "inequality constraints" at \( \x \) and the gradients of the "equality constraints" at \( \x \) form a linearly independent set.
\end{definition}
...
\begin{equation*}
    ...
    \boxed{
        \dotp
        {\pbrac{\func{P_{\calC}}{\x} - \x}}
        {\pbrac{\y - \func{P_{\calC}}{\x}}}
        \geq 0,
        \quad
        \forall \y \in \calC,
        \; \forall \x \in \R^d
...
\begin{itemize}
    \item Either \( \A \x = \b \) has a solution \( \x \geq \zero \), or \( \dotp{\A}{\y} \geq \zero \) has a solution \( \y \in \R^m \) with \( \dotp{\b}{\y} < 0 \).

    \item Either \( \A \x \leq \b \) has a solution \( \x \geq \zero \), or \( \dotp{\A}{\y} \geq \zero \) has a solution \( \y \geq \zero \) with \( \dotp{\b}{\y} < 0 \).

    \item Either \( \A \x \leq \b \) has a solution \( \x \in \R^n \), or \( \dotp{\A}{\y} = \zero \) has a solution \( \y \geq \zero \) with \( \dotp{\b}{\y} < 0 \).

    \item Either \( \A \x = \b \) has a solution \( \x \in \R^n \), or \( \dotp{\A}{\y} = \zero \) has a solution \( \y \in \R^m \) with \( \dotp{\b}{\y} \neq 0 \).
\end{itemize}
...
\newpage
\section{Method of Lagrange multipliers}\label{sec:method-of-lagrange-multipliers}

Consider the "constrained optimisation problem" as in~\psecref{sec:constrained-optimisation-standard-form} with only "equality constraints", i.e., \( \calI = \emptyset \).
The solution corresponding to this problem is always a saddle point of the associated "Lagrangian function".
\begin{equation*}
    \begin{aligned}
        &
        ...
        \func{\nabla_{\x} f}{\xstar}
        +
        \sum_{i \in \calE} \bbrac{\bfmu^\ast}_i \, \func{\nabla_{\x} c_i}{\xstar}
        =
        \zero
    \end{aligned}
    \qquad\qquad
    \begin{aligned}
        &
        \func{\nabla_{\bfmu} \calL}{\xstar, \bfmu^\ast}
        =
        \zero
        \\
        \implies
        ...
        \func{c_i}{\xstar}
        = 0,
        \quad \forall i \in \calE
    \end{aligned}
\end{equation*}

\section{Karush-Kuhn-Tucker (KKT) conditions}\label{sec:kkt-conditions}

\textbf{""KKT conditions""} generalise the method of Lagrange multipliers~\psecref{sec:method-of-lagrange-multipliers} to handle inequality constraints, and are a set of first-order necessary conditions for a solution in nonlinear programming to be optimal, provided that some "regularity conditions" are satisfied.

\begin{theorem}{Karush-Kuhn-Tucker (KKT) conditions / Saddle point theorem~\cite{Nocedal2006,wiki}}{}
    Consider the "constrained optimisation problem" as in~\psecref{sec:constrained-optimisation-standard-form} where \( f, c_i \in \classC^1 \) are continuously differentiable functions.
    Let \( \xstar \) be a local minimum of \( f \) and suppose that "LICQ" holds at \( \xstar \), i.e., the gradients of the "active constraints" at \( \xstar \) are linearly independent.
    Then, there exist Lagrange multipliers \( \bbrac{\bflambda^\ast}_i \geq 0, \; i \in \calE \cup \calI \) such that
    \vspace{-0.5em}
    \begin{align*}
        &
        ...
        \tag{dual feasibility}
        \\ &
        \bbrac{\bflambda^\ast}_i \, \func{c_i}{\xstar}
        = 0,
        \quad \forall i \in \calE \cup \calI
...
Consider the standard form of a "linear programming" problem:
\begin{align*}
    \minimize_{\x}
    \quad &
    \dotp{\c}{\x}
...
\begin{definition}{Basic solution}{}
    A ""basic solution"" is a solution \( \xhat \) of the "LP" problem where at most \( l \) components of \( \xhat \) are non-zero, and the corresponding columns of \( \A \) are linearly independent.
\end{definition}

\section{Basic feasible solution (BFS)}

A ""basic feasible solution"" (BFS) is a "basic solution" that is also feasible for the "LP" problem, i.e., \( \xhat \geq \zero \).

\begin{theorem}{Existence of an optimal BFS}{}
    If the "LP" problem has an optimal solution, then at least one optimal solution is a basic feasible solution.
\end{theorem}

\begin{proof}
    Let \( \xstar \) be an optimal solution of the "LP" problem.
    If \( \xstar \) is a "BFS", then we are done.

    Otherwise, let \( \calI = \set{i \given \bbrac{\xstar}_i > 0, \; i \in \set{1, 2, \ldots, d}} \) be the index set of the non-zero components of \( \xstar \), and let \( r = \abs{\calI} > m \).
    Since the columns of \( \A \) corresponding to the indices in \( \calI \) are linearly dependent, there exists a non-zero vector \( \bfw \in \R^d \) such that
    \begin{equation*}
        \A \bfw = \zero,
        \quad
        \text{and}
        ...
        \bbrac{\bfw}_i = 0,
        \quad
        \forall i \in \set{1, 2, \ldots, d} \setminus \calI
    \end{equation*}

    Consider the family of solutions defined by
    \begin{equation*}
        \func{\x}{\theta}
        =
        \xstar + \theta \, \bfw,
    ...
    \begin{align*}
        ...
        \dotp{\c}{\func{\x}{\theta}}
        & =
        \dotp{\c}{\xstar} + \theta \, \dotp{\c}{\bfw},
        \quad
        \forall \theta \in \R
    \end{align*}

    Since \( \xstar \) is an optimal solution, we must have \( \dotp{\c}{\bfw} = 0\); otherwise, we can choose \( \theta > 0\) or \( < 0\) appropriately to obtain a better solution than \( \xstar \).
    Thus, we have
    \begin{equation*}
        \dotp{\c}{\func{\x}{\theta}}
        =
        ...
        \forall \theta \in \R
    \end{equation*}
    Now, since \( \xstar \geq \zero \), we can choose \( \theta \) such that at least one more component of \( \func{\x}{\theta} \) becomes zero while maintaining the non-negativity of all components.
    By repeating this process, we can eventually obtain a basic feasible solution that is also optimal.
\end{proof}
