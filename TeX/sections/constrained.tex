...
A ""constrained optimisation problem"" can be put into the following standard form:
\begin{align*}
    \minimize_{\x}
    \quad &
    \func{f}{\x}
...
\begin{itemize}
    ...
    \item \AP The constraints \( \func{g_i}{\x} \leq 0 \) are called \textbf{""inequality constraints""}.

    \item \AP The constraints \( \func{h_j}{\x} = 0 \) are called \textbf{""equality constraints""}.
    ...
    \item A \textbf{binding constraint} is a constraint that is satisfied with equality at a given optimal point.
        ...
        \( \text{Binding constraints} \subseteq \text{Active constraints} \), i.e., at an optimal point, all active constraints need not be binding.
        An equality constraint is always binding at an optimal point.
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \quad j \in \set{1, 2, \ldots, p}
    \end{equation*}
    and since \( (\hat{\bflambda}, \hat{\bfmu}) \) is feasible for the dual problem, we have \( \hat{\lambda}_i \geq 0, \; i \in \set{1, 2, \ldots, m} \).
    Thus,
    \begin{equation*}
        \func{q}{\hat{\bflambda}, \hat{\bfmu}}
        =
...
A \textbf{""quadratic programming""} problem is an optimisation problem where the objective function is a quadratic function and the constraints are linear.
\begin{align*}
    \minimize_{\x}
    \quad &
...
\subsection{Convex Equality Quadratic Programming (CEQP)}\label{sec:ceqp}
...
\subsection{Convex Inequality Quadratic Programming (CIQP)}\label{sec:ciqp}
...
\begin{proof}
    ...
    For the CEQP constructed from the active set \( \func{\calA}{\xstar} \), let \( (\xhat, \hat{\bfmu}) \) be its optimal solution, and let \( p = \func{\#}{\func{\calA}{\xstar}} \).
...
\begin{algorithm}[H]
    ...
    \KwIn{
        ...
        Initial active set \( \calW_0 \subseteq \func{\calA}{\x^0} \subseteq \set{1, 2, \ldots, m} \);
    }
    \KwOut{
    ...
    \While{not converged}{
        ...
        \eIf{\( \v_k = \zero \)}{
            \eIf{\( \bfmu_k \geq \zero \)}{
                Optimality conditions met\;

                \Return{\( \x^k, \, \bfmu_k \)\;}
            }{
                Relax the most restrictive active constraint\;
            ...
            \eIf{\( \dotp{\a_i}{\v_k} \geq 0, \; \forall i \in \set{1, 2, \ldots, m} \setminus \calW_k \)}{
                All inactive constraints still remain feasible when taking a full step along \( \v_k \)\;
