...
\begin{align*}
    ...
    \quad j \in \calE
\end{align*}
where \( \calI = \set{1, 2, \ldots, m}, \; \calE = \set{m + 1, m + 2, \ldots, m + p}. \)

\begin{itemize}
    \item The function \( f: \R^d \to \R \) is called the \textbf{objective function}.

    \item The functions \( g_i: \R^d \to \R, \; i \in \calI, \) and \( h_j: \R^d \to \R, \; j \in \calE, \) are called the \textbf{constraint functions}.
    ...
    \item The \textbf{""active set""} at a feasible point \( \x \) is the set of indices of the active constraints at \( \x \), i.e.,
        ...
        \begin{equation*}
            \func{\calA}{\x}
            \triangleq
            \set{i \given \func{g_i}{\x} = 0, \; i \in \calI}
            \; \cup \;
            \calE
        \end{equation*}
