...
\begin{itemize}
    ...
    \item \AP The functions \( c_i: \R^d \to \R, \; i \in \calI \cup \calE \) are called the \textbf{""constraint functions""}.
...
Let \( \xstar \) be a local minimum and suppose that the gradients of the "equality constraints" at \( \xstar \) are linearly independent.
Then, there exist \( \mu_j \in \R, \; j \in \calE \) such that
\begin{equation*}
    \func{\nabla_{\x} \calL}{\xstar, \bfmu}
    =
    \func{\nabla_{\x} f}{\xstar}
    +
...
\begin{theorem}{Karush-Kuhn-Tucker (KKT) conditions / Saddle point theorem~\cite{Nocedal2006,wiki}}{}
    ...
    Then, there exist Lagrange multipliers \( \bbrac{\bflambda^\ast}_i \geq 0, \; i \in \calI \cup \calE \) such that
    \vspace{-0.5em}
    \begin{align*}
        &
        ...
        = 0,
        \quad \forall i \in \calI \cup \calE
