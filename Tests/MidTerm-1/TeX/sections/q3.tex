\section*{Question 3}

Let \( v_{1}, v_{2} \in \mathbb{R}^{d} \) such that \( v_{1}^{\top} v_{2} = 0 \).
Consider \( \min _{\mathbf{x} \in \mathbb{R}^{d}} f(\mathbf{x})\left( = \frac{1}{2}{\left(\mathbf{x}^{\top} v_{1}\right)}^{2}+t v_{2}^{\top} \mathbf{x}\right) \) where \( t \in \mathbb{R} \) is a constant.
Answer the following with justification.
\begin{enumerate}[label= (\alph*)]
    \item Compute the Hessian?
        What are the eigenvalues and eigenvectors of the Hessian?
        Is this function convex?
        Justify.

    \item Compute the global minima of this function for all choices of \( t \in \mathbb{R} \)?
        (Your answer should clearly indicate the optimal value, \( f^{*} \) and the optimal point, \( \mathbf{x}^{*} \)).

    \item Define \( g(\mathbf{x}) = f(\mathbf{x})+\frac{1}{2} {\Vert \mathbf{x} \Vert}^{2} \).
        Repeat a and b.
\end{enumerate}

\subsection*{Solution}
