...
We can see that the relation \( {\u}^\ast = -\pbrac{\bfX \beta^\ast - \y} \) holds true, as can be seen from the residuals values \( \pbrac{\bfX \beta^\ast - \y} \) computed in Q1.4 and the optimal \( {\u}^\ast \) in Q2.3.
